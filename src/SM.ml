...
let rec eval env ((cstack, stack, ((st, i, o) as c)) as conf) = function
  ...
  | insn :: prg' -> 
    match insn with
      ...
      | CJMP (f, s) -> 
        let (Value.Int x)::stack' = stack in 
        let predicate = match f with
          | "z"  -> (==) 0
          | "nz" -> (!=) 0
        in if predicate x then eval env (cstack, stack', c) (env#labeled s) 
                          else eval env (cstack, stack', c) prg'
      
      | CALL (f, n, p) -> 
        if env#is_label f then eval env ((prg', st)::cstack, stack, c) (env#labeled f)
      ...
      | _ -> eval env (match insn with
        | BINOP op -> 
          let y::x::stack' = stack 
          in (cstack, Value.of_int (Expr.to_func op (Value.to_int x) (Value.to_int y)) :: stack', c)
        ...
        | ST x -> 
          let z::stack' = stack 
          in (cstack, stack', (State.update x z st, i, o))
        
        | STA (x, n) -> 
          let v::is, stack' = split (n + 1) stack 
          in (cstack, stack', (Stmt.update st x v @@ List.rev is, i, o))
        
        | LABEL s -> conf
        
        | BEGIN (_, p, l) -> 
          let enter_st = State.enter st (p @ l) in
          let (st', stack') = List.fold_right (
              fun p (st, x::stack') -> (State.update p x st, stack')  
            ) p (enter_st, stack) 
          in (cstack, stack', (st', i, o))
      ) prg'
...
let rec compile' env p =
  ...
  | Stmt.Assign (x, is, e) -> 
    List.concat (List.map expr is) 
    @ expr e 
    @ [STA (x, List.length is)]
  
  | Stmt.Skip -> []
  | Stmt.If (e, s1, s2) -> 
    let fLabel = env#next_label in
    let eLabel = env#next_label 
    in expr e 
     @ [CJMP ("z", fLabel)] 
     @ compile' env s1 
     @ [JMP eLabel; LABEL fLabel] 
     @ compile' env s2 
     @ [LABEL eLabel]
  
  | Stmt.While (e, s) -> 
    let loopLabel = env#next_label in
    let endLabel = env#next_label 
    in [LABEL loopLabel] 
     @ expr e 
     @ [CJMP ("z", endLabel)] 
     @ compile' env s 
